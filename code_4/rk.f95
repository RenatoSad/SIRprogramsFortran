 ...
 implicit none
  ...
  integer(i4b) :: p1,p2,pp2,p3,p4,p5,p6,p7,k,si,ii,ri,jq,i,j,ir,Tr 
  
  real(wp) :: cons0,cons1,cons2,cons3,cons4,cons5
 
  real(wp), dimension(36501,501) :: infec
...
read(8,*) p1,p2,pp2,p3,p4,p5,p6,k,Tr
close(unit=8)
...
call randomcoefci(cons0,cons1,cons2,cons3,cons4,cons5)
...
call eqs2(ks2,ki2,kr2,su,in,re,beta,kappa,kappa2,cons0,cons1,cons2,cons3,cons4,cons5,det,t0 + det/two,om)

  su = so + ks2/2._wp
  in = io + ki2/2._wp
  re = ro + kr2/2._wp
  
call eqs2(ks3,ki3,kr3,su,in,re,beta,kappa,kappa2,cons0,cons1,cons2,cons3,cons4,cons5,det,t0 + det/two,om)

  su = so + ks3
  in = io + ki3
  re = ro + kr3

call eqs2(ks4,ki4,kr4,su,in,re,beta,kappa,kappa2,cons0,cons1,cons2,cons3,cons4,cons5,det,t0,om) 
...
do i = lbound(infec,1), ubound(infec,1)
      write(20,'(1001ES24.13E3)') (infec(i, j), j = lbound(infec,2), ubound(infec,2) )
   end do
...
do i = lbound(Infpr,1), ubound(Infpr,1)
      write(21,'(2ES24.13E3)') (Infpr(i, j), j = lbound(Infpr,2), ubound(Infpr,2) )
   end do
