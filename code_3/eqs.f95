...
subroutine eqs2(s,i,r,so,io,ro,bo,kappa,kappa2,cons0,cons1,cons2,cons3,cons4,cons5,det,t0,omo,j)
 use my_prec, only: i4b,wp => dp
 use dp_const, only: rero,one,two,three,four,zero,zone,zi,pi
 
     real(wp), intent(out) :: r,s,i
     real(wp), intent(in) :: so,io,ro,bo,kappa,kappa2,cons0,cons1,cons2,&
                             cons3,cons4,cons5,t0,det,omo
     real(wp) :: beta
...
s = (-(cons0 * beta * so + cons1 * (beta * so)**2 + cons2 * (beta * so)**3 + cons4 * (beta * so)**4 &
    + cons5 * (beta * so)**5) * io + kappa2 * ro) * det

i = ((cons0 * beta * so + cons1 * (beta * so)**2 + cons2 * (beta * so)**3 + cons4 * (beta * so)**4 &
   + cons5 * (beta * so)**5) * io - kappa * io) * det
...
s = (-bo * (cons0 * beta * so + cons1 * (beta * so)**2 + cons2 * (beta * so)**3 + cons4 * (beta * so)**4 &
    + cons5 * (beta * so)**5) * io + kappa2 * ro) * det

i = (bo * (cons0 * beta * so + cons1 * (beta * so)**2 + cons2 * (beta * so)**3 + cons4 * (beta * so)**4 &
   + cons5 * (beta * so)**5) * io - kappa * io) * det
...
s = (-(cons0 * so + cons1 * (so)**2 + cons2 * (so)**3 + cons4 * (so)**4 &
    + cons5 * (so)**5) * beta * io + kappa2 * ro) * det

i = ((cons0 * so + cons1 * (so)**2 + cons2 * (so)**3 + cons4 * (so)**4 &
   + cons5 * (so)**5) * beta * io - kappa * io) * det
